...
- **Phase-2 ML scorers** — DeepCRISPR on-target (Apache-2.0) is **wired + validated end-to-end** (2026-05-29) against the authors' image (`michaelchuai/deepcrispr:latest`, Python 3.6 / TF 1.13.2): `score_guide_on_target(model="deepcrispr")`, opt-in via `BIOFORGE_DEEPCRISPR_ENABLED`, side-by-side with the rule-based score, graceful when absent. The encoding uses DeepCRISPR's **own** `Sgt` (no reimplementation), and the wrapper emits pure JSON. **FORECasT + Lindel** edit-outcome models are now **validated end-to-end** the same way (2026-05-29) — out-of-process, opt-in, MIT, behind `edit_outcome(model="forecast"|"lindel")`, faithful (raw label->frequency distributions surfaced verbatim, never remapped; `outcomes=[]`), graceful when absent. Lindel runs in `bioforge/lindel:legacy` (pinned `fdcad58`; an **editable** install so `Lindel.__path__` carries the bundled weight pkls; `rev_index = prereq[1]`); FORECasT runs in the thin `bioforge/forecast:legacy` (FROM the authors' `quay.io/felicityallen/selftarget`, base web-server entrypoint cleared) and drops FORECasT's fixed `-` null placeholder so the emitted distribution is over real indels only (see each `models/.../legacy/README.md`). A calibrated accuracy claim (held-out Spearman) and CFD off-target remain.

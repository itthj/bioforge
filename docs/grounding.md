...
- **Phase-2 ML scorers** — DeepCRISPR on-target (Apache-2.0) is **wired + validated end-to-end** (2026-05-29) against the authors' image (`michaelchuai/deepcrispr:latest`, Python 3.6 / TF 1.13.2): `score_guide_on_target(model="deepcrispr")`, opt-in via `BIOFORGE_DEEPCRISPR_ENABLED`, side-by-side with the rule-based score, graceful when absent. The encoding uses DeepCRISPR's **own** `Sgt` (no reimplementation), and the wrapper emits pure JSON. **FORECasT + Lindel** edit-outcome models are scaffolded the same way — out-of-process, opt-in, MIT, behind `edit_outcome(model="forecast"|"lindel")` — with env build + numeric parity still pending (see each `models/.../legacy/README.md`). A calibrated accuracy claim (held-out Spearman) and CFD off-target remain.

...
**Turning on the optional capabilities** (deep models, variant calling, nf-core pipelines, cloud GPU, live benchmarks): see [`docs/READINESS.md`](docs/READINESS.md) — one checklist of which flag / image / data / install lights up each gated feature. The core agent runs on just `ANTHROPIC_API_KEY`; everything else is opt-in and refuses honestly when unconfigured.

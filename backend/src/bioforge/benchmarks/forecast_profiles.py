...
# The observed profiles key blocks `Oligo<N>` while the design library keys `Oligo_<N>` (with an
# underscore); both denote the SAME construct. Canonicalize to `Oligo<N>` so the two join (verified
# live 2026-06-02: ~99.4% of eligible K562 observed oligos join the 41,121-oligo library).
_OLIGO_ID_RE = re.compile(r"Oligo_?(\d+)")
_DNA = set("ACGT")


def _canonical_oligo_id(token: str) -> str | None:
    """Extract the canonical `Oligo<N>` id from a library/observed token, dropping any `_` and
    trailing `_guide` suffix. Returns None if the token carries no `Oligo<N>` id."""
    m = _OLIGO_ID_RE.search(token)
    return f"Oligo{m.group(1)}" if m else None
...
# The target-library registry. The download is bot-gated (PMC reCAPTCHA / Code Ocean login), so the
# file is supplied via `load_target_library(..., local_path=...)`; the sha256 + oligo count below were
# pinned from the file obtained 2026-06-02 from the paper's Code Ocean capsule (data dir).
TARGET_LIBRARY: dict[str, TargetLibrarySpec] = {
    "self_target_oligos": TargetLibrarySpec(
        name="self_target_oligos",
        # Provenance only -- this URL is bot-gated; acquire via the Code Ocean capsule + local_path.
        download_url="https://doi.org/10.24433/CO.6bc7bcae-d736-475b-bae5-00ca0562d401",
        cache_filename="self_target_oligos_details_with_pam_details.csv",
        expected_sha256="d6949518ea73fb72f6dd2f0c41b5c853655ecdca20e64041100d23d4ff53d275",
        n_oligos=41121,
        citation=(
            "Allen F, Crepaldi L, Alsinet C, et al. (2018) Predicting the mutations generated by repair "
            "of Cas9-induced double-strand breaks. Nat Biotechnol 37:64-72. gRNA-target design library "
            "(self_target_oligos_details_with_pam_details.csv) from the Code Ocean capsule "
            "10.24433/CO.6bc7bcae-d736-475b-bae5-00ca0562d401."
        ),
        notes=(
            "Tab-delimited ID/Guide/Target/.../PAM Location/PAM Direction. Library ids are 'Oligo_<N>'; "
            "canonicalized to 'Oligo<N>' to join the observed '@@@Oligo<N>' blocks. Download is "
            "bot-gated -> supply via local_path (sha256-verified)."
        ),
    ),
}
...
def _store_target_fasta(records: dict[str, TargetRecord], header: str, sequence: str) -> None:
    ...
    id_token = tokens[0]
    oid = _canonical_oligo_id(id_token)
    if oid is None:
    ...
    _store_target(records, oid, sequence.upper(), pam_index, tokens[2].upper(), guide)
...
def _parse_target_table(text: str) -> dict[str, TargetRecord]:
    ...
    for lineno, line in enumerate(lines[1:], start=2):
        ...
        oid = _canonical_oligo_id(parts[id_i])
        if oid is None:
        ...
        _store_target(records, oid, parts[tgt_i].strip().upper(), pam_index, direction, guide)
    return records
